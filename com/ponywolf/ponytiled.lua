...
local function tiledProperties(properties)
  if (#properties > 0) and properties[1].name and properties[1].value then
    ...
    for i = 1, #properties do
      if properties[i].value ~= "" then
        t[properties[i].name] = properties[i].value
      end
    end
    return t
  else
...
local function centerAnchor(image, anchorX, anchorY)
  ...
  if image.contentBounds then
    ...
    local actualCenterX, actualCenterY =
      (bounds.xMin * (1 - anchorX) + (bounds.xMax * anchorX)),
...
local function decodeTiledColor(hex)
  ...
  local a, r, g, b =
    hexToFloat(hex:sub(1, 2)),
    hexToFloat(hex:sub(3, 4)),
    hexToFloat(hex:sub(5, 6)),
    hexToFloat(hex:sub(7, 8))
  return r, g, b, a
end
...
function M.new(data, dir)
    ...
    if layer.type == "tilelayer" then
      ...
      for ty = 0, data.height - 1 do
        for tx = 0, data.width - 1 do
          ...
          if gid then
            local image =
              sheet and display.newImage(objectGroup, sheet, gid, 0, 0) or
              display.newImage(objectGroup, dir .. gid, 0, 0)
      ...
      for j = 1, #layer.objects do
        ...
        if object.gid then
          ...
          if gid then
            local image
            image =
              sheet and display.newImageRect(objectGroup, sheet, gid, object.width, object.height) or
              display.newImageRect(
                objectGroup,
                path and path.normalize(dir .. gid) or (dir .. gid),
                object.width,
                object.height
              )
            -- missing
            ...
            if autoShape then
              if not sheet then
                object.properties.outline =
                  graphics.newOutline(autoShape, path and path.normalize(dir .. gid) or (dir .. gid))
              else
        ...
        elseif object.polygon or object.polyline then -- Polygon/line
          ...
          if object.polygon then
            ...
            for p = 1, #points do
              if points[p].x < xMin then
                xMin = points[p].x
              end
              if points[p].y < yMin then
                yMin = points[p].y
              end
              if points[p].x > xMax then
                xMax = points[p].x
              end
              if points[p].y > yMax then
                yMax = points[p].y
              end
            end
          ...
          if polygon.fillColor then
            polygon:setFillColor(decodeTiledColor(polygon.fillColor))
          end
          if polygon.strokeColor then
            polygon:setStrokeColor(decodeTiledColor(polygon.strokeColor))
          end
        elseif object.ellipse then -- circles
          ...
          -- vector properties
          if circle.fillColor then
            circle:setFillColor(decodeTiledColor(circle.fillColor))
          end
          if circle.strokeColor then
            circle:setStrokeColor(decodeTiledColor(circle.strokeColor))
          end
          ...
          -- vector properties
          if rect.fillColor then
            rect:setFillColor(decodeTiledColor(rect.fillColor))
          end
          if rect.strokeColor then
            rect:setStrokeColor(decodeTiledColor(rect.strokeColor))
          end
        end
      end
    end
  ...
  function map:boundsCheck(border)
    ...
    if xMax < display.actualContentWidth then
      self.x = display.contentCenterX
    end
    if yMax < display.actualContentHeight then
